...
- TOAST 场景
- `multi_insert`
- 基于 `archive_dest + pg_wal` 的 WAL 来源自动解析
...
- `pg_flashback.archive_dir`
  - 未设置时，`pg_flashback()` / `fb_export_undo()` 会先报错
  - 当前已接入最小真实 WAL 扫描框架
...
注意：`pg_flashback.archive_dir = '/isoTest/18pgdata/pg_wal'` 只是当前 PG18 基线阶段的开发配置，不是最终产品模型。
...
- WAL 来源解析要同时考虑：
  ...
  - `pg_wal` 已被覆盖、但可通过外部恢复工具补回的 segments
- 对最后一种情况，后续设计会参考 `/root/xman` 中的 `ckwal` 思路，但只作参考，不直接耦合其实现
