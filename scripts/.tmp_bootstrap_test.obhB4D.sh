...
This script:
  setup:
    ...
    4. starts the summary watchdog through scripts/pg_flashback_summary.sh
    5. installs a per-user cron keepalive for the watchdog

  remove:
    1. stops the script-managed summary watchdog and daemon child
    2. removes the bootstrap-installed cron keepalive block
    3. removes generated config / pid / log files
    4. drops the pg_flashback extension
    5. preserves PGDATA/pg_flashback and tells you to remove it manually if needed
...
After bootstrap, manage the daemon manually with start/stop/status:
  scripts/pg_flashback_summary.sh start
  scripts/pg_flashback_summary.sh stop
  scripts/pg_flashback_summary.sh status

Bootstrap also installs a cron-based watchdog keepalive:
  * * * * * scripts/pg_flashback_summary.sh start >/dev/null 2>&1
EOF
}
...
cron_block_installed() {
  ...
  [[ "${current}" == *"${CRON_BLOCK_BEGIN}"* ]] &&
    [[ "${current}" == *"$(cron_command)"* ]] &&
    [[ "${current}" == *"${CRON_BLOCK_END}"* ]]
}
...
install_cron_keepalive() {
  ...
  current=$(read_current_crontab)
  filtered=$(printf '%s\n' "${current}" | strip_bootstrap_cron_block)
  tmp=$(mktemp)
  if [[ -n "${filtered}" ]]; then
    printf '%s\n' "${filtered}" > "${tmp}"
    if [[ "${filtered}" != *$'\n' ]]; then
...
csv_contains_token() {
  ...
  for entry in "${items[@]}"; do
    ...
    if [[ -n "${entry}" && "${entry}" == "${expected}" ]]; then
      return 0
    fi
  done
...
strip_pgflashback_from_preload_file() {
  local path="$1"
  local tmp=""
  ...
  awk '
    ...
    {
      if (match($0, /^[[:space:]]*shared_preload_libraries[[:space:]]*=[[:space:]]*'\''([^'\'']*)'\''([[:space:]]*#.*)?$/, m)) {
        original = m[1]
        suffix = m[2]
        part_count = split(original, parts, ",")
        kept_count = 0
        removed = 0
        delete kept
        ...
        for (i = 1; i <= part_count; i++) {
          ...
          if (token == "")
            continue
          if (token == "pg_flashback") {
            removed = 1
            continue
          }
          kept[++kept_count] = token
        }

        if (removed) {
          if (kept_count > 0)
            print "shared_preload_libraries = '\''" join_kept() "'\''" suffix
          next
        }
      }
...
remove_preloaded_pg_flashback_if_present() {
  local psql_bin="$1"
  local pg_ctl_bin="$2"
  local preload_libraries=""
  local auto_conf="${PGDATA}/postgresql.auto.conf"
  local main_conf="${PGDATA}/postgresql.conf"

  preload_libraries=$(psql_scalar "${psql_bin}" "SELECT current_setting('shared_preload_libraries', true);")
  preload_libraries="$(trim_ascii_whitespace "${preload_libraries}")"
  csv_contains_token "${preload_libraries}" "pg_flashback" || return 0

  stage_detail "detected legacy pg_flashback in shared_preload_libraries; bootstrap will remove it from config files and restart PostgreSQL"
  strip_pgflashback_from_preload_file "${auto_conf}" || true
  strip_pgflashback_from_preload_file "${main_conf}" || true
  run_cmd "${pg_ctl_bin}" -D "${PGDATA}" restart -m fast -w

  preload_libraries=$(psql_scalar "${psql_bin}" "SELECT current_setting('shared_preload_libraries', true);")
  preload_libraries="$(trim_ascii_whitespace "${preload_libraries}")"
  if csv_contains_token "${preload_libraries}" "pg_flashback"; then
    die "shared_preload_libraries still contains pg_flashback after config rewrite; remove it from the remaining PostgreSQL config include chain and rerun setup"
  fi
}
...
main() {
  ...
  [[ ${DRY_RUN} -eq 1 || -x "${pg_ctl_bin}" ]] ||
    die "pg_ctl not found under ${bindir}"
...
watchdog_pid_path=${RUNNER_WATCHDOG_PID_PATH}
...
db_port=${DB_PORT}
pgflashback_data_dir=${PGFLASHBACK_DATA_DIR}
status_command=${RUNNER_SCRIPT} status
EOF
      exit 0
    fi
    ...
    stage_detail "cleaning stale build artifacts for ${PG_CONFIG_BIN}"
    run_cmd make -C "${REPO_ROOT}" "PG_CONFIG=${PG_CONFIG_BIN}" clean
    ...
    remove_preloaded_pg_flashback_if_present "${psql_bin}" "${pg_ctl_bin}"
...
EOF
    ...
    run_cmd "${RUNNER_SCRIPT}" start
    stage "[7/7] Cron Keepalive"
    stage_detail "installing cron keepalive for summary watchdog"
    install_cron_keepalive
...
EOF
  ...
  stage "[2/5] Runner Removal"
  if [[ -f "${config_path}" && -x "${summaryd_bin}" ]]; then
    run_cmd "${RUNNER_SCRIPT}" stop || true
  else
    terminate_summaryd_processes_for_pgdata "${summaryd_bin}"
  fi
  ...
  rm -f "${config_path}" "${RUNNER_WATCHDOG_PID_PATH}" "${RUNNER_PID_PATH}" "${RUNNER_LOG_PATH}" "${summaryd_bin}"

  stage "[3/5] Cron Cleanup"
  stage_detail "removing bootstrap-installed cron keepalive"
  remove_cron_keepalive
