...
DROP TABLE IF EXISTS fb_deep_bag_01;

DO $$
DECLARE
	cols text := 'bucket integer not null';
	col_type text;
	i integer;
BEGIN
	FOR i IN 1..63 LOOP
	...
	EXECUTE format('create table fb_deep_bag_01 (%s)', cols);
	EXECUTE 'alter table fb_deep_bag_01 set (autovacuum_enabled = false, toast.autovacuum_enabled = false)';
END;
$$;
